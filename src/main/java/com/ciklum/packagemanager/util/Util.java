package com.ciklum.packagemanager.util;
...
public final class Util {
    ...
    /**
     * This method checks whether a given class implements Comparable interface.
     * 
     * @param clazz
     *            Specifies the given class.
     * @return True if a given class implements Comparable interface. Otherwise returns false.
     */
    static public boolean isComparable(Class clazz) {
        if (clazz == null) {
            return false;
        }
        ...
        for (Class interfaze : interfaces) {
            if (interfaze.equals(Comparable.class)) {
                return true;
            }
        }
        return isComparable(clazz.getSuperclass());
    }
     ...
     * @return
     */
    public static String listToCommaSeparatedStringWithSmallBraces(List<String> list) {
     ...
     * 
     * @param str
     * @param splitToken TODO
     * @return
     */
    public static List<String> commaSeparatedStringWithSmallBracesToList(String str, String splitToken) {
    	List<String> strList = new ArrayList<String>();
    	if(Util.isNotNullAndEmpty(str)){
			str = str.replace("(", "");
			str = str.replace(")", "");
			for(String type : str.split(splitToken)) {
				strList.add(type);
			}
    	}
    	return strList;
    ...
    /**
     * integerListToStringList
     * 			use convertIntegerToStringList instead.
     * @param list
     * @return
     */
    @Deprecated
    ...
    /**
     * removeTerminators method is used to remove newline characters \n \r from the given string
     * @param input
     * 		specifies the string object
     * @return
     * 		return string without newline characters \n \r
     */
    public static String removeTerminators(String input) {
    	if(Util.isNotNullAndEmpty(input)) {
			input = input.replaceAll("\\r|\\n", " ").replaceAll("<br>", "").replaceAll("<br/>", "").replaceAll("<br />", "").replaceAll("\\s{2,}", " ").replaceAll("\\t", " ")
					.replaceAll("</AddressLine>\\s*<AddressLine>", " ").replaceAll("</span>", "").replaceAll("<span.*?>", "");
    	}
    	return input;
    }
	...
	public static String getCreditCardTypeName(String creditCardTypeNumber){
		
		String typeName = null;
		
		if(creditCardTypeNumber != null){
			
			if("001".equals(creditCardTypeNumber)) {
				typeName = "VISA";
				
			} else if("002".equals(creditCardTypeNumber)) {
				typeName = "MASTER";
				
			} else if("003".equals(creditCardTypeNumber)) {
				typeName = "AMEX";
			}else if ("004".equals(creditCardTypeNumber)) {
				typeName = "DISCOVER";
			}
		}
	...
	public static String getCreditCardTypeStringByNumber(String creditCardNumber){
	...
	public static String getCreditCardTypeCode(String creditCardTypeName) {

		String typeCode = null;

		if (creditCardTypeName != null) {

			if ("VISA".equalsIgnoreCase(creditCardTypeName)) {
				typeCode = "001";

			} else if ("MASTER".equalsIgnoreCase(creditCardTypeName)) {
				typeCode = "002";

			} else if ("AMEX".equalsIgnoreCase(creditCardTypeName)) {
				typeCode = "003";
			}else if ("DISCOVER".equalsIgnoreCase(creditCardTypeName)) {
				typeCode = "004";
			}
		}
	...
	public static Object getFieldValueForObject(String fieldName, Object object) {
		try {
	...
	public static Integer getIdentifierForObject(Object object) {
		Object idValue = getFieldValueForObject("id", object);
		if (idValue != null) {
	...
	public static List<Integer> getIdentifierForObjects(List<Object> objects) {
		List<Integer> identifiers = new ArrayList<Integer>();
		if (Util.isNotNullAndEmpty(objects)) {
			for (Object object : objects) {
				Object idValue = getFieldValueForObject("id", object);
				if (idValue != null) {
					identifiers.add(Integer.parseInt(idValue.toString()));
				}
			}
		}
		return identifiers;
	...
	/***
	 * escapeHtmlEntities method convert some HTML characters or HTML entities back to text 
	 * @param input
	 * 			specify input text which may contains some HTML entities e.g James&#x27;s => James's
	 * @return input
	 */
	

	public static String replaceTerminatorsWithCharacter(String input) {
		if (Util.isNotNullAndEmpty(input)) {
			input = input.replaceAll("</[F|f]irst[N|n]ame>\\s*<[L|l]ast[N|n]ame>", "/").replaceAll("\\(infant[^\\)]+\\)", "").replaceAll("(?:\\s*[\\d]{2}(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)\\sOUT\\s*|\\s*[\\d]{1,}\\sNIGHT[S]*\\s*|\\s*[\\d]{1,}\\sROOM\\s*)", " ").trim();
		}
		return input;
	}
	
	public static String getSecureJsonString(String string) {
		
		if (Util.isNotNullAndEmpty(string)) {
			String encryptedString = string.replaceAll("\"creditCardNumber\"\\s*:\\s*\"[0-9]+(?=[0-9]{4}\")", "\"creditCardNumber\":\"************")
					.replaceAll("\"creditCardCcv\"\\s*:\\s*\"[0-9]+\"", "\"creditCardCcv\":\"****\"")
					.replaceAll("\"ccv\"\\s*:\\s*\"[0-9]+\"", "\"ccv\":\"****\"")
					.replaceAll("\"password\"\\s*:\\s*\"(.*?)\"", "\"password\":\"********\"")
					.replaceAll("\"tax\"\\s*:\\s*\"(.*?)\"", "\"tax\":\"**-*******\"")
					.replaceAll("\"taxId\"\\s*:\\s*\"(.*?)\"", "\"taxId\":\"**-*******\"")
					.replaceAll("\"ssn\"\\s*:\\s*\"(.*?)\"", "\"ssn\":\"***-**-****\"")
					.replaceAll("\"profilePhoto\"\\s*:\\s*\"(.*?)\"", "\"profilePhoto\":\"image\"");
